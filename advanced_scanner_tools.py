...
class ScannerTools:
    ...
    def ping_tool(self):
        ...
        count_combo = ttk.Combobox(count_frame, textvariable=count_var,
                                   values=[_ for _ in range(1,17)], width=10, state='normal')
        ...
        size_combo = ttk.Combobox(size_frame, textvariable=size_var,
                                  values=["32", "64", "128", "256", "512", "1024"], width=10, state='normal')
        ...
        timeout_combo = ttk.Combobox(timeout_frame, textvariable=timeout_var,
                                     values=["1", "2", "3", "5", "10"], width=10, state='normal')
        ...
        def validate(host, number, size, timeout):
            # Checking host

            shell_metachars = ['&', '|', ';', '$', '`', '>', '<', '!', '\n', '\r', '\t',
                               '(', ')', '[', ']', '{', '}', '*', '?', '~', '#', '@', '%',
                               '\\', '"', "'"]
            for i in shell_metachars:
                if i in host:
                    self.log(
                        f'[WARNING] Injection Detected! Info: \nhost: {host}, \nnumber: {number}, \nsize: {size}, \ntimeout: {timeout}',
                        "injection")
                    return
            ...
            try:
                ...
                print(f"[Number] Only numbers are supported.")
                self.log("[Number] ValueError", 'injection')
                tk.messagebox.showwarning("[Number] ValueError", "[Number] Only numbers are supported. Don't try to hack yourself!")
                return

            if number < 1 or number > (2 ** 100 + 1):
                print(f"[Number] Number must be between 1 and (2**100 + 1).")
                self.log("[Number] ValueError", 'BigNumber')
                tk.messagebox.showwarning("[Number] ValueError", "[Number] Number must be between 1 and (2**100 + 1).")
                return
            number = int(number)
            ...
            try:
                ...
                print(f"[Size] Only numbers are supported.")
                self.log("[Size] ValueError", 'injection')
                tk.messagebox.showwarning("[Size] ValueError", "[Size] Only numbers are supported.")
                return
            size = int(size)
            ...
            try:
                ...
                print(f"[Size] Only numbers are supported.")
                self.log("[Timeout] ValueError", 'injection')
                tk.messagebox.showwarning("[Timeout] ValueError", "[Timeout] Only numbers are supported.")
                return
            timeout = int(timeout)

            return host, number, size, timeout
        ...
        def do_ping():
            ...
            if not validate(host, count, size, timeout):
                return
            ...
            try:
                if platform.system().lower() == "windows":
                    cccmd = ['ping', '-n', count, '-l', size, '-w', str(int(timeout) * 1000), host]
                    cmd = f'chcp 65001 > nul && {" ".join(cccmd)}'
                else:
...
<body>
    ...
    def traffic_analysis(self):
        ...
        def analyze_traffic():
            ...
            try:
                if platform.system() == "Windows":
                    cmd = ['netstat', '-e']
                    cmd = f"chcp 65001 > null && {' '.join(cmd)}"
                    result = subprocess.run(cmd, capture_output=True, text=True, shell=True)
    ...
    def ssl_check(self):
        ...
        def check_ssl():
            if not tk.messagebox.askokcancel("Предупреждение", "Запуск проверки SSL может привести к длительному зависанию приложения. Вы точно хотите продолжить?"):
                return
    ...
    def whois_lookup(self):
        ...
        def do_whois():
            if not tk.messagebox.askokcancel("Предупреждение",
                                             "Запуск проверки SSL может привести к длительному зависанию приложения. Вы точно хотите продолжить?"):
                return
    ...
    def network_mapper(self):
        """Сканер сети с обнаружением устройств и их характеристик"""
        dialog = tk.Toplevel()
        dialog.title("Карта сети")
        dialog.geometry("800x600")

        ttk.Label(dialog, text="Карта сети", font=('Arial', 14, 'bold')).pack(pady=10)

        main_frame = ttk.Frame(dialog)
        main_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

        # Дерево устройств
        tree_frame = ttk.LabelFrame(main_frame, text="Обнаруженные устройства", padding="10")
        ...
        self.devices_tree.heading('hostname', text='Имя хоста')
        self.devices_tree.heading('mac', text='MAC адрес')
        self.devices_tree.heading('vendor', text='Производитель')
        self.devices_tree.heading('os', text='ОС')
        self.devices_tree.heading('open_ports', text='Открытые порты')

        self.devices_tree.column('ip', width=120)
        self.devices_tree.column('hostname', width=150)
        self.devices_tree.column('mac', width=120)
        self.devices_tree.column('vendor', width=150)
        self.devices_tree.column('os', width=100)
        self.devices_tree.column('open_ports', width=100)
        ...
        ttk.Button(btn_frame, text="Сканировать сеть",
                   command=self.start_network_mapping).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Экспорт в CSV",
                   command=self.export_network_map).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Генерировать отчет",
    ...
    def vulnerability_scanner(self):
        ...
        if not tk.messagebox.askokcancel("Предупреждение",
            ...
            return
        dialog = tk.Toplevel()
        dialog.title("Сканер уязвимостей")
        dialog.geometry("700x500")

        ttk.Label(dialog, text="Сканер уязвимостей", font=('Arial', 14, 'bold')).pack(pady=10)
        ...
        ttk.Label(settings_frame, text="Цель:").grid(row=0, column=0, sticky=tk.W, pady=5)
        target_entry = ttk.Entry(settings_frame, width=30)
        target_entry.grid(row=0, column=1, pady=5, padx=5)
        target_entry.insert(0, "192.168.1.1")

        ttk.Label(settings_frame, text="Профиль сканирования:").grid(row=1, column=0, sticky=tk.W, pady=5)
        profile_var = tk.StringVar(value="Базовый")
        profile_combo = ttk.Combobox(settings_frame, textvariable=profile_var,
                                     values=["Базовый", "Полный", "Безопасный", "Агрессивный"],
                                     state='readonly', width=20)
        profile_combo.grid(row=1, column=1, pady=5, padx=5)
        ...
        def start_scan():
            ...
            results_text.insert(tk.END, f"Начато сканирование уязвимостей для {target}\n")
            results_text.insert(tk.END, f"Профиль: {profile}\n\n")

            # Симуляция сканирования
            vulnerabilities = [
                {"name": "Weak SSH Keys", "severity": "HIGH", "description": "Слабые ключи SSH"},
                {"name": "Outdated Apache", "severity": "MEDIUM", "description": "Устаревшая версия Apache"},
                {"name": "Open FTP Port", "severity": "LOW", "description": "Открытый FTP порт без аутентификации"},
            ]

            for vuln in vulnerabilities:
                results_text.insert(tk.END, f"[{vuln['severity']}] {vuln['name']}\n")
                results_text.insert(tk.END, f"    {vuln['description']}\n\n")

            results_text.insert(tk.END, "✓ Сканирование завершено\n")

        btn_frame = ttk.Frame(dialog)
        btn_frame.pack(pady=10)

        ttk.Button(btn_frame, text="Начать сканирование",
                   command=start_scan).pack(side=tk.LEFT, padx=5)
    ...
    def packet_sniffer(self):
        ...
        if not tk.messagebox.askokcancel("Предупреждение",
            ...
            return
        dialog = tk.Toplevel()
        dialog.title("Сниффер пакетов")
        dialog.geometry("900x700")

        ttk.Label(dialog, text="Анализатор сетевого трафика", font=('Arial', 14, 'bold')).pack(pady=10)
        ...
        self.packet_count = ttk.Label(stats_frame, text="Пакетов: 0")
        self.packet_count.pack(side=tk.LEFT, padx=20)

        self.tcp_count = ttk.Label(stats_frame, text="TCP: 0")
        self.tcp_count.pack(side=tk.LEFT, padx=20)

        self.udp_count = ttk.Label(stats_frame, text="UDP: 0")
        self.udp_count.pack(side=tk.LEFT, padx=20)
        ...
        self.packets_tree.heading('no', text='№')
        self.packets_tree.heading('time', text='Время')
        self.packets_tree.heading('source', text='Источник')
        self.packets_tree.heading('destination', text='Назначение')
        self.packets_tree.heading('protocol', text='Протокол')
        self.packets_tree.heading('length', text='Длина')
        self.packets_tree.heading('info', text='Информация')

        for col in columns:
            self.packets_tree.column(col, width=100)

        scrollbar = ttk.Scrollbar(packets_frame, orient=tk.VERTICAL, command=self.packets_tree.yview)
        ...
        # Подробности пакета
        details_frame = ttk.LabelFrame(main_frame, text="Подробности пакета", padding="10")
        ...
        ttk.Button(btn_frame, text="Начать захват",
                   command=start_sniffing).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Остановить",
                   command=stop_sniffing).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Сохранить в файл",
                   command=self.save_packets).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Закрыть",
                   command=dialog.destroy).pack(side=tk.LEFT, padx=5)
    ...
    def port_forwarding_tool(self):
        """Инструмент проброса портов"""
        if not tk.messagebox.askokcancel("Предупреждение",
                                         "Эта функция ещё не реализована. Вы точно хотите продолжить?"):
            return
        dialog = tk.Toplevel()
        dialog.title("Проброс портов")
        dialog.geometry("600x500")

        ttk.Label(dialog, text="Настройка проброса портов", font=('Arial', 14, 'bold')).pack(pady=10)
        ...
        ttk.Label(settings_frame, text="Локальный порт:").grid(row=0, column=0, sticky=tk.W, pady=5)
        local_port = ttk.Entry(settings_frame, width=10)
        local_port.grid(row=0, column=1, pady=5, padx=5)
        local_port.insert(0, "8080")

        ttk.Label(settings_frame, text="Удаленный хост:").grid(row=1, column=0, sticky=tk.W, pady=5)
        remote_host = ttk.Entry(settings_frame, width=20)
        remote_host.grid(row=1, column=1, pady=5, padx=5)
        remote_host.insert(0, "192.168.1.100")

        ttk.Label(settings_frame, text="Удаленный порт:").grid(row=2, column=0, sticky=tk.W, pady=5)
        remote_port = ttk.Entry(settings_frame, width=10)
        remote_port.grid(row=2, column=1, pady=5, padx=5)
        remote_port.insert(0, "80")

        ttk.Label(settings_frame, text="Протокол:").grid(row=3, column=0, sticky=tk.W, pady=5)
        protocol_var = tk.StringVar(value="TCP")
        ttk.Combobox(settings_frame, textvariable=protocol_var,
                     values=["TCP", "UDP"], state='readonly', width=10).grid(row=3, column=1, pady=5, padx=5)

        # Статус
        status_frame = ttk.Frame(main_frame)
        status_frame.pack(fill=tk.X, pady=10)

        self.forwarding_status = ttk.Label(status_frame, text="Не активно", foreground="red")
        self.forwarding_status.pack()
        ...
        def start_forwarding():
            ...
            try:
                ...
                rh = remote_host.get()
                rp = int(remote_port.get())
                proto = protocol_var.get()

                self.forwarding_status.config(text=f"Активно: {lp} -> {rh}:{rp}", foreground="green")

                messagebox.showinfo("Успех",
                                    f"Проброс портов настроен:\nЛокальный порт {lp} -> {rh}:{rp}\nПротокол: {proto}")

            except ValueError:
                messagebox.showerror("Ошибка", "Введите корректные номера портов")

        def stop_forwarding():
            self.forwarding_status.config(text="Не активно", foreground="red")

        btn_frame = ttk.Frame(dialog)
        btn_frame.pack(pady=10)

        ttk.Button(btn_frame, text="Запустить проброс",
                   command=start_forwarding).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Остановить",
                   command=stop_forwarding).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Тестировать",
                   command=self.test_forwarding).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Закрыть",
                   command=dialog.destroy).pack(side=tk.LEFT, padx=5)

    def test_forwarding(self):
        """Тестирование проброса портов"""
        messagebox.showinfo("Тест", "Функция тестирования проброса портов")
        tk.messagebox.showerror("Ошибка", "Функция ещё не реализована!")
        return
    ...
    def subdomain_scanner(self):
        ...
        if not tk.messagebox.askokcancel("Предупреждение",
            ...
            return
        dialog = tk.Toplevel()
        dialog.title("Сканер поддоменов")
        dialog.geometry("700x600")

        tk.messagebox.showinfo("Функция недоступна", "Функция ещё не реализована полностью.")

        ttk.Label(dialog, text="Поиск поддоменов", font=('Arial', 14, 'bold')).pack(pady=10)
        ...
        ttk.Label(settings_frame, text="Домен:").grid(row=0, column=0, sticky=tk.W, pady=5)
        domain_entry = ttk.Entry(settings_frame, width=30)
        domain_entry.grid(row=0, column=1, pady=5, padx=5)
        domain_entry.insert(0, "example.com")

        ttk.Label(settings_frame, text="Словник:").grid(row=1, column=0, sticky=tk.W, pady=5)
        wordlist_var = tk.StringVar(value="Основной")
        ttk.Combobox(settings_frame, textvariable=wordlist_var,
                     values=["Основной", "Большой", "Маленький", "Пользовательский"],
                     state='readonly', width=15).grid(row=1, column=1, pady=5, padx=5)

        ttk.Label(settings_frame, text="Потоки:").grid(row=2, column=0, sticky=tk.W, pady=5)
        threads_spin = ttk.Spinbox(settings_frame, from_=1, to=50, width=10)
        threads_spin.grid(row=2, column=1, pady=5, padx=5)
        threads_spin.set("10")

        # Результаты
        results_frame = ttk.LabelFrame(main_frame, text="Найденные поддомены", padding="10")
        results_frame.pack(fill=tk.BOTH, expand=True)

        self.subdomains_list = tk.Listbox(results_frame, height=15)
        scrollbar = ttk.Scrollbar(results_frame, orient=tk.VERTICAL, command=self.subdomains_list.yview)
        self.subdomains_list.configure(yscrollcommand=scrollbar.set)

        self.subdomains_list.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        ...
        def start_scan():
            ...
            threads = threads_spin.get()

            self.subdomains_list.delete(0, tk.END)
            self.subdomains_list.insert(tk.END, f"Начато сканирование поддоменов для {domain}")

            # Симуляция поиска поддоменов
            common_subdomains = [
                "www", "mail", "ftp", "admin", "blog", "shop", "api",
                "test", "dev", "staging", "secure", "portal", "vpn"
            ]

            for sub in common_subdomains:
                self.subdomains_list.insert(tk.END, f"{sub}.{domain}")

            self.subdomains_list.insert(tk.END, f"✓ Найдено {len(common_subdomains)} поддоменов")
        ...
        ttk.Button(btn_frame, text="Начать сканирование",
                   command=start_scan).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Сохранить результаты",
                   command=self.save_subdomains).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Проверить доступность",
                   command=self.check_subdomains).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Закрыть",
                   command=dialog.destroy).pack(side=tk.LEFT, padx=5)
        tk.messagebox.showerror("Ошибка", "Функция ещё не реализована!")

    def save_subdomains(self):
        """Сохранение найденных поддоменов"""
        tk.messagebox.showinfo("Функция недоступна", "Функция ещё не реализована.")
        pass
    ...
    def wifi_scanner(self):
        ...
        wifi_scanner.WiFiScanner().create_scanner_window()



    def password_cracker_tool(self):
        """Инструмент для тестирования паролей (только для образовательных целей!)"""
        if not tk.messagebox.askokcancel("Предупреждение",
                                         "Эта функция ещё не реализована. Вы точно хотите продолжить?"):
            return
        if not tk.messagebox.askokcancel("Предупреждение",
                                         "ВАЖНО: Используйте этот инструмент только на своих системах!"):
            return
        dialog = tk.Toplevel()
        dialog.title("Тестер паролей")
        dialog.geometry("600x400")

        ttk.Label(dialog, text="ПРЕДУПРЕЖДЕНИЕ: Только для легального тестирования!",
                  font=('Arial', 10, 'bold'), foreground="red").pack(pady=5)

        ttk.Label(dialog, text="Тестер надежности паролей", font=('Arial', 12)).pack(pady=5)

        main_frame = ttk.Frame(dialog)
        main_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

        ttk.Label(main_frame, text="Хост:").grid(row=0, column=0, sticky=tk.W, pady=5)
        host_entry = ttk.Entry(main_frame, width=20)
        host_entry.grid(row=0, column=1, pady=5, padx=5)
        host_entry.insert(0, "192.168.1.1")

        ttk.Label(main_frame, text="Порт:").grid(row=1, column=0, sticky=tk.W, pady=5)
        port_entry = ttk.Entry(main_frame, width=10)
        port_entry.grid(row=1, column=1, pady=5, padx=5)
        port_entry.insert(0, "22")

        ttk.Label(main_frame, text="Протокол:").grid(row=2, column=0, sticky=tk.W, pady=5)
        protocol_var = tk.StringVar(value="SSH")
        ttk.Combobox(main_frame, textvariable=protocol_var,
                     values=["SSH", "FTP", "HTTP Basic", "Telnet"],
                     state='readonly', width=15).grid(row=2, column=1, pady=5, padx=5)

        ttk.Label(main_frame, text="Логин:").grid(row=3, column=0, sticky=tk.W, pady=5)
        login_entry = ttk.Entry(main_frame, width=20)
        login_entry.grid(row=3, column=1, pady=5, padx=5)
        login_entry.insert(0, "admin")

        ttk.Label(main_frame, text="Словник паролей:").grid(row=4, column=0, sticky=tk.W, pady=5)
        wordlist_var = tk.StringVar(value="Базовый")
        ttk.Combobox(main_frame, textvariable=wordlist_var,
                     values=["Базовый", "Расширенный", "Пользовательский"],
                     state='readonly', width=15).grid(row=4, column=1, pady=5, padx=5)

        output_text = scrolledtext.ScrolledText(main_frame, height=8)
        output_text.grid(row=5, column=0, columnspan=2, pady=10, sticky=tk.NSEW)
        ...
        def test_passwords():
            ...
            if not tk.messagebox.askokcancel("Предупреждение",
                                             "ВАЖНО: Используйте этот инструмент только на своих системах!"):
                return
            tk.messagebox.showwarning("INFO", "ВАЖНО: Используйте этот инструмент только на своих системах!")
            output_text.delete(1.0, tk.END)
            output_text.insert(tk.END, "Тестирование паролей начато...\n\n")
            output_text.insert(tk.END, "ВАЖНО: Используйте этот инструмент только на своих системах!\n")
            output_text.insert(tk.END, "Тестирование стандартных паролей...\n\n")

            common_passwords = ["admin", "password", "123456", "admin123", "root"]

            for pwd in common_passwords:
                output_text.insert(tk.END, f"Пробуем: {pwd} ... НЕ ПОДОШЕЛ\n")

            output_text.insert(tk.END, "\n✓ Тестирование завершено\n")
            output_text.insert(tk.END, "Используйте сложные пароли для защиты!\n")

        btn_frame = ttk.Frame(dialog)
        btn_frame.pack(pady=10)

        ttk.Button(btn_frame, text="Начать тестирование",
                   command=test_passwords).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Закрыть",
                   command=dialog.destroy).pack(side=tk.LEFT, padx=5)
